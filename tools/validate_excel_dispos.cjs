...
function parseFrDate(val) {
  ...
  const monthMap = { janvier:'01', fevrier:'02', février:'02', mars:'03', avril:'04', mai:'05', juin:'06', juillet:'07', aout:'08', août:'08', septembre:'09', octobre:'10', novembre:'11', decembre:'12', décembre:'12' }
...
function buildDateBlocks(aoa, headerRow){
  const dateRow = headerRow - 1
  const headers = aoa[headerRow] || []
  const dates = aoa[dateRow] || []
  const dateCols = []
  for (let c=0;c<dates.length;c++) { const d = parseFrDate(dates[c]); if (d) dateCols.push({col:c,date:d}) }
  const blocks = []
  for (const {col,date} of dateCols){
    const windowEnd = Math.min(headers.length - 1, col + 6)
    const slice = headers.slice(col, windowEnd+1).map(x=>String(x||'').toLowerCase())
    const local = { date }
...
function main(){
  ...
  if(!fileArg){ console.error('Usage: node tools/validate_excel_dispos.cjs <fichier.xlsx>'); process.exit(1) }
  const full = path.resolve(fileArg)
  if(!fs.existsSync(full)){ console.error('Fichier introuvable:', full); process.exit(1) }
  const wb = XLSX.readFile(full, { cellDates:false })
  let sheetName='Dispos'; if(!wb.Sheets[sheetName]) sheetName = wb.SheetNames[0]
  ...
  if(!week39Dates.length) console.log('⚠️ Aucune date de la semaine 39 détectée')
  else console.log('✅ Semaine 39 couverte')
}
